...
function cntBuildCmd(cnt) {
  ...
  let extPath = cntExtPath;
  if (!extPath) {
    // Will be resolved at shell execution time
    extPath = ""; // handled by VOID_EXT variable below
  }
  ...
  // Pre-enable developer mode and write container name marker
  const prefsFile = `${dataDir}/Default/Preferences`;
  lines.push(`mkdir -p "${dataDir}/Default"`);
  ...
  // TODO: --load-extension doesn't work reliably on all Chrome versions
  // User may need to manually load the extension via chrome://extensions in container windows
  lines.push(`google-chrome --user-data-dir="${dataDir}" --no-first-run --no-default-browser-check --install-autogenerated-theme="${rgb}" \${VOID_EXT:+--load-extension="$VOID_EXT"} "${url}" &`);
