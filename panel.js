...
function cntBuildCmd(cnt) {
  ...
  // Navigate to a setup page first that registers the container name, then redirect to target
  const extId = chrome.runtime.id;
  const setupUrl = `chrome-extension://${extId}/container-setup.html?name=${encodeURIComponent(safeName)}&redirect=${encodeURIComponent(url)}`;
  lines.push(`google-chrome --user-data-dir="${dataDir}" --no-first-run --no-default-browser-check --install-autogenerated-theme="${rgb}" \${VOID_EXT:+--load-extension="$VOID_EXT"} "${setupUrl}" &`);
