...
const detailSuffixes = [
  "detail-close","detail-title","detail-to-rep","detail-to-intr","detail-poc","detail-notes",
  "detail-open","reflect-bar","reflect-hl","reflect-chips","detail-render","detail-curl",
...
for (const p of detailPrefixes) for (const s of detailSuffixes) ids.push(p+"-"+s);
...
["hist","tgt","ep","log","sens"].forEach(p=>{const b=detailSuffixes.filter(s=>s.startsWith("detail-")).map(s=>s.replace("detail-",""));t(p,rq.filter(x=>!b.includes(x)).length===0);});
